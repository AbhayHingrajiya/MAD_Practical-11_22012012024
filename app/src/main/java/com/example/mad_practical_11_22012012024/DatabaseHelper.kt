...
class DatabaseHelper(context: Context?) :
    ...
    fun insertPerson(Contect: Contect): Long {
        // get writable database as we want to write data
        val db = this.writableDatabase
        // insert row
        val id = db.insert(PersonDbTableData.TABLE_NAME, null, getValues(Contect))
        // close db connection
        db.close()
        // return newly inserted row id
        return id
    }
    private fun getValues(Contect: Contect): ContentValues {
        val values = ContentValues()
        // `id` will be inserted automatically.
        // no need to add them
        values.put(PersonDbTableData.COLUMN_ID, Contect.id)
        values.put(PersonDbTableData.COLUMN_PERSON_NAME, Contect.name)
        values.put(PersonDbTableData.COLUMN_PERSON_EMAIL_ID, Contect.emailId)
        values.put(PersonDbTableData.COLUMN_PERSON_PHONE_NO, Contect.phoneNo)
        values.put(PersonDbTableData.COLUMN_PERSON_GPS_LAT, Contect.latitude)
        values.put(PersonDbTableData.COLUMN_PERSON_ADDRESS, Contect.address)
        values.put(PersonDbTableData.COLUMN_PERSON_GPS_LONG, Contect.longitude)
        return values
    }
    fun getPerson(id: String): contect? {
